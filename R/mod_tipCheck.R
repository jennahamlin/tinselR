...
#'
#' @noRd 
mod_tipCheck_server <- function(input, output, session, geneFileTipCheckOut, geneSep){
  ns <- session$ns
  ...
  sanity <- function(impGene, genSep ) {
    
    # #tree <- treeio::read.newick(file = impTree) #this is the imported tree - impTree
    # treeIn <- impTree
    # str(treeIn$tip.label)
    # print(treeIn$tip.label)
    # print("line 56")
    ...
    # #treeTips <- sort(treeIn$tip.label)
     ...
     gFile <- readData(filePath = impGene$datapath, sep = genSep) #fileIn is either tsv, txt, or csv of genetic distance and sep is a reactive selected by user and is the variable of impGene
     #str(gFile)
